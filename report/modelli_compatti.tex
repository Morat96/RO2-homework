...
Here we introduce different formulations for the traveling salesman problem that we implemented. For all the following models we will use the decision variable $x_{ij}$ already defined in 1.2.\\
...
Constraints \ref{eqn:mtz-constraint} ensure that, if the salesman travels from i to j, then the position of node j is one more than that of node i. This allows to have only a polynomial number of variable and constraints. In total, there are $O(n^2)$ variables and $O(n^2)$ constraints. The problem is no more NP-Hard, but the relaxation of the problem is somewhat weak and as one can see in the section where compact models are compared, it yields to decent performances only for small datasets.
...
The idea is that whenever one node is chosen, the salesman delivers one unity of commodity flow. In this way, an order between the nodes is picked and subtours are impossible.\\
...
Constraints \ref{eqn:f2-const-1} and \ref{eqn:f2-const-2} force $(n-1)$ units of commodity 1 to flow in at city 1 and 1 unit of commodity to flow out at every other node.\\
Constraits \ref{eqn:f2-const-3} and \ref{eqn:f2-const-4} force $(n-1)$ units of commodity 2 to flow out at city 1 and 1 unit of commodity to flow in at every other node.\\
...
Constraints \ref{eqn:fm-const-1} only allow flow in an arc which is present. Constraints \ref{eqn:fm-const-2} force exactly one unit of each commodity to flow in at city 1 and constraints \ref{eqn:fm-const-3} does the opposite, preventing commodity to flow out at city 1.\\
Constraints \ref{eqn:fm-const-4} force exactly one unity of commodity k to flow out at city k and constraints \ref{eqn:fm-const-5} prevent any of commodity k flowing in at city k.\\
...
As explained in the previous sections, we decided mainly to focus on MTZ and Flow Based formulations, because of the time required to compute the optimal solution for the datasets we used.\\
We tested these algorithms on datasets with a number of nodes up to 80. For bigger datasets, the computation of an optimal solution became unfeasible.\\
