...
We used two variables to manage the time limit of the algorithm. The variable \textit{remaining\_time} keeps track of the time left to the algorithm to compute the solution. At the beginning the variable assumes the value of the original time limit given by the user (line 2). The variable \textit{time\_x\_cycle} is the inner time limit and it is initially fixed to \textit{time\_limit/5} (line 3). Since with the inner time limit it is very hard that an initial good solution will be computed, we provide to CPLEX a heuristic solution builded with the GRASP algorithm (see Section \ref{sec:grasp}) and refined with the 2-OPT algorithm (see Section \ref{two-opt}) (lines 4 - 6).
...
\begin{algorithm}
	...
	\If{$ (\textit{remaining\_time} \leq 0)$} 
	\State \textbf{break}
	\EndIf
